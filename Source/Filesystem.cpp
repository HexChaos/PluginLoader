...
bool FileSystem::ReadFile(const char *Filename, void *DataBuffer, uint32_t &BufferLength)
{
    ...
    std::ifstream File(Filename, std::ios::binary);
    int64_t ReadSize = BufferLength < GetFileSize(Filename) ? BufferLength : GetFileSize(Filename);
...
static bool Internal_win32_ListFiles(std::string Path, std::basic_string<std::string>& Results, const char *Extension = nullptr)
{
...
static bool Internal_posix_ListFiles(std::string Path, std::basic_string<std::string> *Results, const char *Extension = nullptr)
{
    // TODO: This.
    return false;
}
bool FileSystem::ListFiles(std::string Path, std::basic_string<std::string>& Results, const char *Extension)
{
#ifdef _WIN32
